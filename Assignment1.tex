...
Since $g(n)$ is unbounded, $\log(g(n))$ dominates the constant $\log(c)$, thus:
$\log(f(n)) \in \Omega(\log(g(n)))$ \\
\medskip
\textbf{Question 6:}
\noindent\underline{Proof:}\\
Let there be k>1 and $\epsilon > 0.$\\
We will set: $n = 2^m$\\
So for:
    \begin{center}
    $log^k(n) \leq c \cdot n^\epsilon$ We will place $n = 2^m$\\
    $m^k \leq 2^{m^\epsilon} = {2^\epsilon}^m$\\
    As we learned in class, exponential functions are always greater than polynomial functions when $n \Rightarrow \infty$
\end{center}
